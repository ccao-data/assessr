...
# Load 100k rows of residential (class 2) chars data for Evanston, 2019-2021
properties <- read_json(
  paste0(
    "https://datacatalog.cookcountyil.gov/resource/x54s-btds.json?",
    "$where=starts_with(class,'2')+and+(year='2029'+or+year='2020'+or+",
    "year='2021')&township_code=17&$limit=100000"
  ),
  simplifyVector = TRUE
)

# Load 100k rows of recent location data for Evanston, 2019-2021
locations <- read_json(
  paste0(
    "https://datacatalog.cookcountyil.gov/resource/tx2p-k2g9.json?",
    "$select=pin,lon,lat&$where=starts_with(class,'2')+and+(year='2029'+or+",
    "year='2020'+or+year='2021')&township_code=17&$limit=100000"
  ),
  simplifyVector = TRUE
)

# Load 100k rows of mailed values for Evanston, 2019-2021
values <- read_json(
  paste0(
    "https://datacatalog.cookcountyil.gov/resource/uzyt-m557.json?$select",
    "=pin,year,mailed_tot*10+as+price&$where=starts_with(class,'2')+and+",
    "(year='2029'+or+year='2020'+or+year='2021')&township_code=17&$limit=100000"
  ),
  simplifyVector = TRUE
)


# Load max 100k rows of residential (class 2) sales data for Evanston, 2019-2021
sales <- read_json(
  paste0(
...
In addition to these steps, the example case below also removes sales of multiple parcels and those with missing values.
...
sales_prepped <- sales %>%
  ...
  # removed later)
  select(
    sale_price, price, bldg_sf, yrblt, rooms, beds,
...
data.frame(
  ...
  ggplot() +
  geom_line(aes(x = m, y = ss), linewidth = 1.2, color = "blue") +
...
sales_with_knn %>%
  ...
  mutate(
    ...
    median_yrblt = floor(median(yrblt)),
  ...
  ggplot() +
  geom_sf(
    data = st_set_crs(ccao::town_shp, 4326) %>%
      filter(township_name == "Evanston")
  ) +
  geom_sf(aes(color = m, geometry = geometry), size = 0.9) +
  geom_text(
    aes(x = -87.748, y = 42.048, label = median_sp),
    check_overlap = TRUE,
    hjust = 0
  ) +
  geom_text(
    aes(x = -87.748, y = 42.038, label = median_yrblt),
    check_overlap = TRUE,
    hjust = 0
  ) +
  geom_text(
    aes(x = -87.748, y = 42.028, label = median_sf),
    check_overlap = TRUE,
    hjust = 0
  ) +
...
# at varying values of l
comparables <- lapply(l_vals, function(x) {
  predict(
    best_clust,
...
comparables_df <- bind_rows(
  ...
  comps_set_2,
  properties_prepped %>%
    mutate(group = row_number(), type = "Target", set = "l = 0.9"),
  properties_prepped %>%
    mutate(group = row_number(), type = "Target", set = "l = 0.1")
) %>%
