...
comparables_small %>%
  group_by(set, group) %>%
  summarize(
    across(bldg_sf:beds, function(x) {
      fx <- first(x)
      sums <- sum(between(x, fx - (fx * 0.1), fx + (fx * 0.1)))
      sums <- (sums - 1) / (n() - 1)
    }),
  ...
  scale_fill_distiller(
    name = paste(
      "Percent of comparable characteristics\nwithin",
      "10% of target property's value*"
    ),
    direction = 1,
...
comparables_small %>%
  ...
  ggplot() +
  geom_sf(
    data = st_set_crs(ccao::nbhd_shp, 4326) %>%
  ...
  labs(caption = paste(
    "*Sub-boundaries represent",
    "Cook County Assessor neighborhoods"
  )) +
...
comparables_small %>%
  ...
  geom_vline(
    aes(xintercept = median_sp, color = "Median", linetype = "Median")
  ) +
